...
public class BeanUtils {
	...
	private static class Property {
		...
		public Method getRead() throws IllegalAccessException {
			if (read == null) {
				throw new IllegalAccessException(String.format("%s 的属性 %s read 方法不存在", clazz.getName(), name));
			}
			return read;
		}
		...
		public Method getWrite() throws IllegalAccessException {
			if (write == null) {
				throw new IllegalAccessException(String.format("%s 的属性 %s write 方法不存在", clazz.getName(), name));
			}
			return write;
		}
	}
	...
	private Property _getProperty(Object bean, String property, boolean maybeNull) throws IntrospectionException {
		...
		if (p == null) {
			...
			if (p == null && !maybeNull) {
				throw new RuntimeException(String.format("%s 的属性 %s 不存在", clazz.getName(), property));
			}
		}
		return p;
	}
